...
fn main() {
    ...
    for s in lambert(r1_km, r2_km, tof_s, MU_EARTH_KM3_S2, false, 0).unwrap() {
        println!(
            "  M={} iters={} x={:+.6}\n    v1_km_s = [{:+.6}, {:+.6}, {:+.6}]\n    v2_km_s = [{:+.6}, {:+.6}, {:+.6}]\n    |v1|={:.4} km/s  |v2|={:.4} km/s",
    ...
    for s in lambert(r1_km, r2_km, tof_s, MU_EARTH_KM3_S2, true, 0).unwrap() {
        println!(
    ...
    for s in lambert(r1_km, r2_km, tof_s, MU_SUN_KM3_S2, false, 0).unwrap() {
        println!(
    ...
    for s in lambert(r1_km, r2_km, tof_s, MU_EARTH_KM3_S2, false, 3).unwrap() {
        println!(
            "  M={} iters={} x={:+.6}  |v1|={:.4} km/s  |v2|={:.4} km/s",
            s.n_revs, s.iters, s.x, s.v1_km_s.norm(), s.v2_km_s.norm(),
        );
